...
type FileUploadFieldProp<T extends FieldValues, K extends Path<T> = Path<T>> = {
  ...
  accept?: string
  mapUploadResponse?: (data: unknown, file: File) => DocumentMetaData
...
export default function FileUploadField<T extends FieldValues>({
  ...
  const initialFile = useMemo(() => {
    ...
    return {
      file: new File([], verifiedValue.name, {
        type: getMimeTypeFromUrl(verifiedValue.url),
      }),
  ...
  const {
    ...
    uploadInfo,
    cancelUpload,
  } = useUploadRequest<DocumentMetaData>({
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="h-[20px] px-2 flex items-center border border-border rounded">
              <span className="text-[11px] text-muted-foreground">
                {formatFileSize(
                  Number(uploadedFile.metadata?.size) || uploadedFile.file.size,
                )}
              </span>
            </div>
            <label
    ...
    <div className="relative">
      ...
      >
        ...
        {isRequired ? (
          ...
          <h3 className="font-medium my-2 text-foreground text-center">
            {uploadTitle}
          </h3>
        )}
        ...
        <input
          accept={
            accept ??
            (mediaType === 'all' ? 'image/*, application/*' : `${mediaType}/*`)
          }
        ...
        />
        {error ? (
          <span className="text-destructive text-sm absolute -bottom-5 left-2">
            {error}
          </span>
        ) : null}
      </label>
